...
export default function ModalFadeArticle({ toggleModal, article }: ModalFadeArticleProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const url = article ? `http://localhost:5000/api/article/${article.id}` : "http://localhost:5000/api/article";
      ...
      if (!res.ok) throw new Error(article ? "Gagal mengedit artikel" : "Gagal menambahkan artikel");

      Swal.fire({ title: article ? "Artikel Berhasil Diedit!" : "Artikel Berhasil Ditambahkan!", icon: "success" });
      toggleModal(); // Tutup modal setelah sukses
    } catch (error) {
      console.error("Error:", error);
      Swal.fire({ icon: "error", title: "Oops...", text: article ? "Terjadi kesalahan saat mengedit artikel." : "Terjadi kesalahan saat menambahkan artikel." });
    }
  };
      ...
      <div className="relative p-4 w-full max-w-lg max-h-full">
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 p-6">
            <div>
              <label htmlFor="judul" className="block text-sm font-medium text-gray-900 dark:text-white">Judul</label>
              <input
                type="text"
                name="judul"
                id="judul"
            ...
            <div>
              <label htmlFor="tanggal" className="block text-sm font-medium text-gray-900 dark:text-white">Tanggal</label>
              <input
                type="date"
                name="tanggal"
                id="tanggal"
            ...
            <div>
              <label htmlFor="gambar" className="block text-sm font-medium text-gray-900 dark:text-white">Gambar</label>
              <input
                type="file"
                name="gambar"
                id="gambar"
            ...
            <div>
              <label htmlFor="isi" className="block text-sm font-medium text-gray-900 dark:text-white">Isi</label>
              <textarea
                name="isi"
                id="isi"
                value={isi}
